...
class Actor(nn.Module):
    def __init__(
        ...
        action_dim: int   = 2,
        hidden_dim: int   = 64,
        map_half:   float = 3.0,           # match env.max_offset; was 10.0 (wasted dynamic range)
        log_std_init: float = -0.5,        # std=0.6 instead of 1.0 — less wild exploration
    ):
        super().__init__()
        self.map_half = map_half           # NOTE: legacy name kept; semantically this is action_limit
