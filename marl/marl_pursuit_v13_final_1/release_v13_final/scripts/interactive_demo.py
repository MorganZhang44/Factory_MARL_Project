...
# ── Config ────────────────────────────────────────────────────────────
DEFAULT_CFG_PATH  = "configs/mappo_role_intruder.yaml"
DEFAULT_CKPT_PATH = "results/checkpoints_role_300k/final.pt"
...
def draw_debug_panel(screen, font, font_bold, font_big, font_small, dbg):
    ...
    sg_age   = dbg.get("sg_age", [0, 0])
    sg_max   = dbg.get("sg_commit_frames", 10)
    _kv(screen, font, px, y, "sg age A1/A2:",
        f"{sg_age[0]}/{sg_max}  {sg_age[1]}/{sg_max}"); y += 18
...
def main():
    ...
    # ── Subgoal commit policy ────────────────────────────────────────
    # Subgoals are held across frames so the agent commits to a direction
    # for ~0.5 s instead of having a fresh subgoal every tick. They're
    # also allowed to be farther (cap at SG_FALLBACK_CAP for the closing
    # fallback so ideal slots up to 6 m can be the active subgoal).
    SG_COMMIT_FRAMES   = 10      # hold subgoal for at least 10 frames (0.5 s)
    ...
    SG_POLICY_CAP      = 3.0     # m — policy subgoal max (matches training)
    ...
    def reset_episode():
        ...
        return {
            ...
            "close_frames": 0,
            "capture_hold": 0,
            # per-agent committed subgoal state
            "committed_sg":         [None, None],
            "sg_age":               [0, 0],
            "sg_committed_target":  [None, None],
            "was_locked":           [False, False],
        }
    ...
    while running:
        ...
        # ── Move agents ───────────────────────────────────────────────
        # Subgoals are committed for ≥ SG_COMMIT_FRAMES frames so we don't
        # re-aim every tick. Recommit triggers: lock just released, no
        # commit yet, age expired, agent reached subgoal, or target moved
        # significantly since commit.
        for i in range(env.n_agents):
            if lock_flags[i]:
                ...
                env._paths[i] = []
                # Invalidate committed subgoal so next non-lock frame
                # recommits fresh.
                state["committed_sg"][i]        = None
                state["sg_age"][i]              = 0
                state["sg_committed_target"][i] = None
                state["was_locked"][i]          = True
                continue

            # Choose magnitude cap based on which controller produced the action
            ...
            candidate_sg = np.clip(candidate_sg, -MAP_HALF + 0.3, MAP_HALF - 0.3)

            # Decide: reuse committed subgoal or recommit?
            should_recommit = False
            committed = state["committed_sg"][i]
            if committed is None:
                should_recommit = True
            elif state["was_locked"][i]:
                should_recommit = True
            elif state["sg_age"][i] >= SG_COMMIT_FRAMES:
                should_recommit = True
            elif float(np.linalg.norm(env.agent_pos[i] - committed)) <= SG_REACHED_THRESH:
                should_recommit = True
            else:
                committed_target = state["sg_committed_target"][i]
            ...
            if should_recommit:
                state["committed_sg"][i]        = candidate_sg.copy()
                state["sg_age"][i]              = 0
                state["sg_committed_target"][i] = env.target_pos.copy()
                state["was_locked"][i]          = False
            else:
                state["sg_age"][i] += 1

            sg = state["committed_sg"][i]
            env._subgoals[i] = sg
        ...
        dbg = {
            ...
            "sg_age":          list(state.get("sg_age", [0, 0])),
