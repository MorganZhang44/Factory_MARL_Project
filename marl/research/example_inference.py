...
CFG_PATH  = RESEARCH_ROOT / "configs" / "mappo_role_intruder_300k.yaml"
CKPT_PATH = RESEARCH_ROOT / "checkpoints" / "v13_final.pt"
...
for step in range(3):
    ...
    print(f"      capture_geom_now   : {info.get('capture_geom_now', False)}")
    print(f"      capture_hold       : {info.get('capture_hold', 0)}/{info.get('capture_hold_target', 20)}  (frames; 20 = 2 s)")
