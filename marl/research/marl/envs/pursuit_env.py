...
class PursuitEnv(gym.Env):
    ...
    def __init__(self, cfg: dict, render_mode: Optional[str] = None):
        ...
        # Differential-drive constraints for agents:
        #   max angular velocity (rad/s) — body rotates gradually, can't flip 180°
        #   movement is locked to the body's current heading direction.
        self.agent_max_omega   = float(env_cfg.get("agent_max_omega", 0.6))   # rad/s
        # Capture condition extension: each agent's heading must face the intruder
        # (cos between heading and (target − pos) ≥ this threshold). 0.5 ≈ 60° cone.
        self.face_capture_thresh = float(env_cfg.get("face_capture_thresh", 0.5))
        # Capture lock-in: geometric capture conditions must hold continuously
        # for this many steps (e.g. 20 = 2 s at dt=0.1) before capture fires.
        self.capture_hold_steps = int(env_cfg.get("capture_hold_steps", 20))
        ...
        self.agent_yaw  = np.zeros(self.n_agents,    dtype=np.float64)   # heading [rad]
        ...
        self._stuck_steps         = 0
        # Capture-hold counter: counts consecutive steps that the full geometric
        # capture condition (radius + angle + facing) is satisfied. Resets on
        # any miss. Capture is finalised only when ≥ capture_hold_steps.
        self._capture_hold        = 0
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.agent_vel[:] = 0.0
        # Initial heading: each agent faces the intruder (zero rotation needed at t=0)
        for i in range(self.n_agents):
            d = self.target_pos - self.agent_pos[i]
            self.agent_yaw[i] = float(np.arctan2(d[1], d[0])) if np.linalg.norm(d) > 1e-3 else 0.0
    ...
    def step(self, actions: np.ndarray, intruder_action: Optional[np.ndarray] = None):
        ...
        for i in range(self.n_agents):
            ...
            (self.agent_pos[i],
             self.agent_vel[i],
             self.agent_yaw[i]) = self._move_along_path(
                self.agent_pos[i], self._paths[i], self.agent_max_spd,
                other_pos=other_pos,
                current_yaw=float(self.agent_yaw[i]),
                max_omega=self.agent_max_omega,
            )
        ...
        # 5. Termination check
        #    Geometric conditions (instantaneous):
        #      (a) both agents within capture_radius
        #      (b) angle between (P→target) and (E→target) ≥ 90°  (encirclement)
        #      (c) both agents' heading faces the intruder, cos ≥ face_capture_thresh
        #    Capture (terminating) requires (a)+(b)+(c) to hold for `capture_hold_steps`
        #    consecutive frames — i.e. the agents must lock onto the intruder for ≥2 s.
        ...
        if self.n_agents >= 2:
            ...
            if both_in_capture:
                ...
                if cos_theta <= 0.0:
                    facing_ok = True
                    for i in range(2):
                        head = np.array([np.cos(self.agent_yaw[i]),
                                         np.sin(self.agent_yaw[i])])
                        to_t = self.target_pos - self.agent_pos[i]
                        n_to = float(np.linalg.norm(to_t))
                        if n_to < 1e-3:
                            continue
                        if float(np.dot(head, to_t / n_to)) < self.face_capture_thresh:
                            facing_ok = False
                            break
                    if facing_ok:
                        capture_geom_now = True
        else:
            capture_geom_now = bool(np.any(dists_after <= self.capture_radius))

        # Increment / reset the lock-in timer
        if capture_geom_now:
            self._capture_hold += 1
        else:
            self._capture_hold = 0
        captured = (self._capture_hold >= self.capture_hold_steps)
        ...
        if self.use_roles:
            rewards = self.reward_fn.compute_per_agent(
                ...
                capture_geom_now=capture_geom_now,
            )
        else:
        ...
        info = {
            "captured": captured,
            "capture_geom_now": capture_geom_now,
            "capture_hold": int(self._capture_hold),
            "capture_hold_target": int(self.capture_hold_steps),
    ...
    def _move_along_path(
        ...
        Differential-drive style:
          • Compute desired heading (toward next path waypoint).
          • If `current_yaw` is given AND `max_omega > 0`, the heading rotates
            by at most `max_omega · dt` rad per step toward the desired one.
          • Movement is then locked to the (possibly limited) heading direction.
          • Translation magnitude = `max_spd · dt` — agents move a constant
            forward distance every frame; they cannot side-step.

        Wall and agent-agent collisions trigger the X / Y axis slide fallback.

        Returns: (new_pos, new_vel, new_yaw)
        """
        if not path:
            yaw_out = float(current_yaw) if current_yaw is not None else 0.0
            return pos.copy(), np.zeros(2), yaw_out
        ...
        # Apply angular velocity limit if this agent has a tracked yaw
        if current_yaw is not None and max_omega > 0.0:
            yaw_err = desired_yaw - float(current_yaw)
            # Wrap to [-π, π]
            yaw_err = (yaw_err + np.pi) % (2.0 * np.pi) - np.pi
            max_step_yaw = max_omega * self.dt
            yaw_err = float(np.clip(yaw_err, -max_step_yaw, max_step_yaw))
            new_yaw = float(current_yaw) + yaw_err
            # Wrap new_yaw to [-π, π] for numeric stability
            new_yaw = (new_yaw + np.pi) % (2.0 * np.pi) - np.pi
            unit = np.array([np.cos(new_yaw), np.sin(new_yaw)])
        else:
            new_yaw = desired_yaw
            unit = diff / dist

        # Forward speed is capped by max_spd × dt, BUT also by remaining
        # distance to the next waypoint — this lets the policy slow down /
        # stop by issuing small action magnitudes (subgoal close to current
        # position). Without this cap the agent is forced to move every
        # frame and can never hold position to satisfy the capture lock.
        step    = min(max_spd * self.dt, dist)
        vel     = unit * (step / self.dt)
        new_pos = pos + unit * step

        min_sep = 2.0 * self.agent_radius + 0.1   # 0.7 m hard separation
        ...
        if _blocked(new_pos):
            ...
            if not _blocked(slide_x):
                return slide_x, np.array([vel[0], 0.0]), new_yaw
            slide_y = np.array([pos[0], new_pos[1]])
            if not _blocked(slide_y):
                return slide_y, np.array([0.0, vel[1]]), new_yaw
            # Stuck — keep yaw update so agent can still rotate in place
            return pos.copy(), np.zeros(2), new_yaw

        return new_pos, vel, new_yaw
    ...
    def _step_intruder_rl(self, action: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Intruder has no yaw constraint — pass current_yaw=None, max_omega=0
        new_pos, new_vel, _ = self._move_along_path(self.target_pos, path, self.intruder_spd)
