# MARL Pursuit-and-Encircle (`research`)

This directory contains the preserved `v13` research and training tree that was
split out from the runtime-facing `marl/` module. It is meant for:

- MAPPO training,
- offline evaluation,
- interactive demos,
- policy/reward/environment inspection.

It is not the primary online service boundary. The online runtime continues to
use:
...
The rest of this document is the original release-oriented `v13` README.
...
python marl/research/scripts/interactive_demo.py \
    --config marl/research/configs/mappo_role_intruder_300k.yaml \
    --checkpoint checkpoints/v13_final.pt

# 3. Programmatic example (no pygame, prints obs/action/info for 3 frames)
python marl/research/example_inference.py

# 4. Headless capture-rate benchmarks (no GUI)
python marl/research/scripts/eval_static.py         --checkpoint checkpoints/v13_final.pt --episodes 30
python marl/research/scripts/eval_demo_fallback.py  --episodes 100      # closing-fallback only
python marl/research/scripts/eval_combined.py       --checkpoint checkpoints/v13_final.pt --episodes 50
```
...
├── configs/
│   ├── mappo_config.yaml                    ← current shared MAPPO config
│   ├── mappo_role_intruder_300k.yaml        ← compatibility alias for research scripts
│   └── mappo_role_intruder_smoke.yaml       ← compatibility alias for smoke/eval scripts
├── scripts/
...
```bash
PYTHONUTF8=1 PYTHONIOENCODING=utf-8 python marl/research/scripts/interactive_demo.py ...
```
...
cfg  = yaml.safe_load(open("marl/research/configs/mappo_role_intruder_300k.yaml", encoding="utf-8"))
...
### 8.3 Activation tiers (`marl/research/scripts/interactive_demo.py`)
...
python marl/research/scripts/train_mappo.py \
    --config marl/research/configs/mappo_role_intruder_smoke.yaml \
    --device cpu \
