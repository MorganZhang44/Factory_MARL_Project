"""
eval_demo_case.py — replicates the exact demo failure mode:
  - intruder stationary
  - intruder near a wall
  - both agents spawn on the SAME SIDE of the intruder (within 90° wedge)

This is the hardest case the policy faces, and matches what the user sees
when they wave the mouse around then stop near a wall.
"""
...
def main():
    ...
    cfg["env"]["intruder_near_obstacle_prob"] = 1.0
    cfg["env"]["same_side_spawn_prob"] = 1.0  # ALWAYS same-side
    ...
    for ep in range(args.episodes):
        ...
        print(f"  Ep {ep+1:>2}/{args.episodes}  cap={info['captured']}  "
              f"steps={info['step']:>3}  min={ep_min:.2f}m  fin={info['min_dist']:.2f}m")

    print()
    print(f"DEMO-CASE eval (stationary + near-wall + same-side spawn, {args.episodes} eps):")
