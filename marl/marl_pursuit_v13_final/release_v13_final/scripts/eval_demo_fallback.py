...
def step_with_lock(env, actions, lock_flags):
    """Mirrors the demo's main loop motion logic, with lock-flag bypass."""
    env._step_count += 1
    ...
    for i in range(env.n_agents):
        ...
        if mag > env.agent_max_off:
            raw = (raw / mag) * env.agent_max_off
        sg = env.agent_pos[i] + raw
        sg = np.clip(sg, -env.map_half + 0.3, env.map_half - 0.3)
        env._subgoals[i] = sg
