...
def _depth_payload(msg: Image) -> dict[str, Any]:
    ...
    if finite.size > 0:
        ...
        image = values.reshape((int(msg.height), int(msg.width))).astype(np.float32, copy=False)
        valid_mask = np.isfinite(image) & (image > 0.0)
        if np.any(valid_mask):
            valid_values = image[valid_mask]
            lo = float(np.percentile(valid_values, 5))
            hi = float(np.percentile(valid_values, 95))
            if hi <= lo:
                hi = lo + 1e-3
            ...
            max_width = 320
            pil_image = PillowImage.fromarray(preview, mode="L")
            if pil_image.width > max_width:
                new_height = max(1, int(pil_image.height * max_width / pil_image.width))
                pil_image = pil_image.resize((max_width, new_height))
