...
function renderRobotPage(data) {
  ...
  for (const [robotId, robot] of Object.entries(data.robots || {})) {
    ...
    const depthHtml = depth.image ? `<img src="${depth.image}" alt="${robotId} depth" />` : `<span class="mono">waiting for depth</span>`;
    ...
    card.innerHTML = `
      ...
      <div class="robot-layout">
        <div class="stack">
          ...
          <div>
            <div class="subheading">Depth</div>
            <div class="image-box">${depthHtml}</div>
          </div>
          <div>
