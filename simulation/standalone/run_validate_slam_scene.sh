...
CONDA_ENV="${ISAAC_CONDA_ENV:-isaaclab51}"
...
if ! conda env list | awk '{print $1}' | grep -qx "${CONDA_ENV}"; then
  ...
  echo "Set ISAAC_CONDA_ENV to the Isaac environment you want to use, for example:" >&2
  echo "  ISAAC_CONDA_ENV=isaaclab51 bash simulation/standalone/run_validate_slam_scene.sh --headless" >&2
  exit 1
fi
