...
from environment_rewrite.config.scene_cfg import SurveillanceSceneCfg
print("[DEBUG] Importing sensor_cfg...", flush=True)
from environment_rewrite.config.sensor_cfg import get_intrinsics_from_camera_cfg, SURVEILLANCE_CAM_CFG, DOG_CAM_CFG
print("[DEBUG] Importing scene_manager...", flush=True)
from environment_rewrite.scene.surveillance_scene import SurveillanceSceneManager
...
from environment_rewrite.localization_mesh import create_static_localization_mesh
...
def _load_shared_slam_scene(scene_usd_path: str = SLAM_SCENE_USD) -> None:
    """Reference the shared SLAM scene USDA file under /World/SlamScene.

    The walls, obstacles, floor, and lights are defined in this single USDA
    asset (mirrored by share/slam_scene.usda from the simulation module).
    SimulationContext already creates the active stage physics scene, so the
    USDA reference must not keep its own PhysicsScene active as well.
    """
    import omni.usd

    try:
        import isaacsim.core.utils.prims as prim_utils
    except ImportError:  # older Isaac Sim builds
        import omni.isaac.core.utils.prims as prim_utils

    if not os.path.exists(scene_usd_path):
        raise FileNotFoundError(
            f"SLAM scene USDA file not found: {scene_usd_path}"
        )

    prim_utils.create_prim("/World", "Xform")
    prim_utils.create_prim("/World/SlamScene", "Xform", usd_path=scene_usd_path)
    prim_utils.create_prim("/World/Actors", "Xform")

    # Keep the referenced PhysicsScene inactive. The live stage physics scene
    # comes from SimulationContext, and leaving both active triggers PhysX
    # multi-scene stepping warnings / unstable behavior.
...
def setup_scene() -> tuple[InteractiveScene, SimulationContext]:
    ...
    # Load walls/obstacles/floor/lights from the shared SLAM scene file.
    _load_shared_slam_scene()
    print(f"[INFO] Loaded SLAM scene from {SLAM_SCENE_USD}", flush=True)
    ...
    # Create scene (only dogs, suspect, sensors, and CCTV cameras now;
    # walls/obstacles/floor/lights come from the loaded USDA file).
...
def main():
    ...
    log_file.write(
        "step,time,detected,est_x,est_y,est_z,gt_x,gt_y,gt_z,error_m,confidence,n_cams,n_lidars,"
        "est_vx,est_vy,est_vz,gt_vx,gt_vy,gt_vz,speed_est,speed_gt,vel_error_mps,speed_error_mps\n"
    )
    ...
    for step in range(args.num_steps):
        ...
        if step % sensor_interval == 0:
            ...
            est_v = fusion_result.velocity_world
            suspect_asset_for_log = get_entity(scene, "suspect")
            if suspect_asset_for_log is not None:
                gt_vel_w = suspect_asset_for_log.data.root_lin_vel_w[0]
            else:
                gt_vel_w = torch.zeros(3, device=gt_pos.device)
            if est_v is not None:
                est_v_dev = est_v.to(gt_vel_w.device)
                vel_err = float(torch.norm(est_v_dev[:2] - gt_vel_w[:2]).item())
                speed_err = abs(float(torch.norm(est_v_dev[:2]).item() - torch.norm(gt_vel_w[:2]).item()))
                est_v_str = f"{est_v[0]:.3f},{est_v[1]:.3f},{est_v[2]:.3f}"
                speed_est_val = float(torch.norm(est_v_dev[:2]).item())
            else:
                vel_err = float("nan")
                speed_err = float("nan")
                est_v_str = "nan,nan,nan"
                speed_est_val = float("nan")
            speed_gt_val = float(torch.norm(gt_vel_w[:2]).item())
            gt_v_str = f"{gt_vel_w[0]:.3f},{gt_vel_w[1]:.3f},{gt_vel_w[2]:.3f}"
            ...
            log_file.write(
                ...
                f"{est_v_str},{gt_v_str},"
                f"{speed_est_val:.4f},{speed_gt_val:.4f},{vel_err:.4f},{speed_err:.4f}\n"
            )
