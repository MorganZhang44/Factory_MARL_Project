...
if ! conda env list | awk '{print $1}' | grep -qx "${CONDA_ENV}"; then
  ...
  echo "Simulation rewrite must run in its own Isaac Sim environment, default: isaaclab51." >&2
  exit 1
fi
...
exec conda run --no-capture-output -n "${CONDA_ENV}" python simulation/standalone/run_environment_rewrite.py "${ARGS[@]}"
