...
class RobotMirror:
    ...
    status: TimedValue = field(default_factory=TimedValue)
...
class StateMirror:
    ...
    def update_robot_status(self, robot_id: str, payload: dict[str, Any], now_sec: float) -> None:
        with self._lock:
            robot = self._state.robots[robot_id]
            robot.status = _next_timed_value(robot.status, payload, now_sec)
    ...
    def update_camera_payload(self, robot_id: str, payload: dict[str, Any], now_sec: float) -> None:
        with self._lock:
            robot = self._state.robots[robot_id]
            robot.camera = _next_timed_value(robot.camera, payload, now_sec)

    def update_camera_jpeg(self, robot_id: str, jpeg_bytes: bytes, frame_id: str, now_sec: float) -> None:
        payload = _camera_payload_from_jpeg(jpeg_bytes, frame_id)
        if payload is None:
            return
        self.update_camera_payload(robot_id, payload, now_sec)
    ...
    def snapshot(self, include_images: bool = True, max_lidar_points: int = 0) -> dict[str, Any]:
        now_sec = time.monotonic()
        with self._lock:
            robots = {
    ...
    def _robot_snapshot(
        ...
        return {
            ...
            "status": self._timed_snapshot(robot.status, now_sec),
            "camera": camera,
...
def _camera_payload_from_jpeg(jpeg_bytes: bytes, frame_id: str) -> dict[str, Any] | None:
    try:
        pil_image = PillowImage.open(io.BytesIO(jpeg_bytes)).convert("RGB")
    except Exception:
        return None

    original_width = int(pil_image.width)
    original_height = int(pil_image.height)
    max_width = 320
    if pil_image.width > max_width:
        new_height = max(1, int(pil_image.height * max_width / pil_image.width))
        pil_image = pil_image.resize((max_width, new_height))
